...
** Last update Mon May  9 14:20:27 2016 louis-emile uberti-ares
*/
...
static char	**malloc_tab(char **tab, int *lenght, int nb_words)
{
  ...
  while (i < nb_words)
    {
      if ((tab[idx] = bunny_malloc(lenght[idx] + 1)) == NULL)
	return (NULL);
...
char		**my_str_to_wordtab(const char *str, const char *sep)
{
  ...
  if ((nb_words == 0) ||
      ((tab = bunny_malloc(sizeof(char *) * (nb_words + 1))) == NULL) ||
      ((lenght = bunny_malloc(sizeof(int) * nb_words)) == NULL))
    return (NULL);
