...
// Last update Fri Apr  7 20:49:17 2017 Lous-Emile Uberti-Ares
//

#include "Sfml.hh"

arcade::Sfml::Sfml(int width, int height, const std::string &name)
{
...
void			arcade::Sfml::renderText(const std::string &text, const Vec2i &pos,
						 const int size,
...
void			arcade::Sfml::loadTexture(const std::vector<std::string> &filepath)
{
...
void			arcade::Sfml::deleteSpritesAndTextures()
{
  this->_texture.clear();
  this->_sprite.clear();
}

void			arcade::Sfml::renderTexture(const std::string &filepath,
						    const Vec2i &pos,
						    const Vec2i &dim,
...
void			arcade::Sfml::clearScreen() const
{
  this->_window->clear(sf::Color::Black);
}

void			arcade::Sfml::updateScreen() const
{
  this->_window->display();
}

arcade::Event		arcade::Sfml::getLastEvent() const
{
  sf::Event		event;
...
{
  return (new arcade::Sfml(dim.x, dim.y, name));
}
